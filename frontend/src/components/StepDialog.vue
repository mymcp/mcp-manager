        ...
        <n-form ref="formRef" :model="formValue" :rules="rules">
          ...
          <template v-if="selectedInstallation">
            ...
            <template v-if="selectedArguments">
              <n-form-item
                v-for="(param, key) in selectedArguments"
                :key="key"
                :label="key"
                :path="`arguments.${key}`"
...
function handleTypeChange(type) {
  ...
  // 初始化参数
  if (selectedServer.value?.arguments) {
    Object.keys(selectedServer.value.arguments).forEach(key => {
