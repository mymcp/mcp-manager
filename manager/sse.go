...
func AddProxyRoute(instanceId string, proxyServer *proxy.ProxyServer) {
	httpMutex.Lock()
	...
	// 保存SSE服务器
	sseServers[instanceId] = sseServer

	httpRouter.Handle(sseServer.CompleteSsePath(), sseServer)
	httpRouter.Handle(sseServer.CompleteMessagePath(), sseServer)

	log.Printf("Serve %s[%s] on %s\n", proxyServer.ServerInfo, instanceId, sseServer.CompleteSseEndpoint())
}

func RemoveProxyRoute(instanceId string) {
	httpMutex.Lock()
	defer httpMutex.Unlock()

	sseServer, ok := sseServers[instanceId]
	if !ok {
		return
	}
	// 移除ProxyRoute
